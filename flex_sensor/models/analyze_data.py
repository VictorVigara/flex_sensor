...
def plot_sensor_outputs(
    data, orientations, positions, data_folder_path, model_type, normalized=False
):
    ...
    for i, orientation in enumerate(unique_orientations):
        ...
        axs[i].set_title(f"Orientation {orientation}°")
    ...
    plt.savefig(
        os.path.join(
            data_folder_path,
            f'sensor_outputs_all_orientations{"_normalized" if normalized else ""}.png',
        )
    )
    plt.show()
...
def radial_plot(
    ...
    ax.set_title("Radial Plot of Sensor Outputs")
    ...
    plt.savefig(
        os.path.join(
            data_folder_path,
            f'radial_plot_{model_type}{"_normalized" if normalized else ""}.png',
        )
    )
    plt.show()
