...
class CNN_FNN_Raw_Diff_Dataset(Dataset):
    def __init__(self, raw_values, diff_values, angles, displacements, force_applied):
        self.raw_values = torch.tensor(raw_values, dtype=torch.float32).reshape(
            -1, 1, 2, 2
...
class CNN_FNN_Raw_Diff(nn.Module):
    ...
    def forward(self, raw_values, diff_values):
        x = torch.relu(self.conv1(raw_values))
        x = torch.relu(self.conv2(x))
        x = x.view(x.size(0), -1)  # Flatten the tensor
        x_raw = torch.relu(self.fc1_raw(x))
        ...
        x_combined = torch.cat((x_raw, x_diff), dim=1)

        # Forward pass for combined network
        x = torch.relu(self.fc_combined(x_combined))
...
if __name__ == "__main__":
    ...
    dataset = CNN_FNN_Raw_Diff_Dataset(
        all_data, sensor_differences, all_orientations, all_positions, force_applied
    )
